...
def add_padding(input_tensor):
    ...
    return tf.concat([input_tensor, tf.zeros([1, dim])], axis=0)
...
class BoxEMulti:
    ...
    def train_with_valid(self, separate_valid_model=True, print_period=1, epoch_ckpt=50, save_period=1000,
        ...
        if self.use_tensorboard:
            ...
            # 1. Costruiamo il nome originale
            summary_descriptor = str(self.kb_name) + "_" + str(self.stop_gradient) + "_" + str(self.learning_rate) + \
                                ...
                                "neg_opt-" + str(self.neg_sampling_opt) + "_" + time.asctime()
            
            # 2. PULIAMO IMMEDIATAMENTE la variabile summary_descriptor 
            # Sostituiamo caratteri non validi (spazi, :, /, etc) con underscore
            summary_descriptor = re.sub(r'[^a-zA-Z0-9\-_]', '_', summary_descriptor)
            
            # 3. Ora usiamo questa variabile pulita per tutto
            path_to_summary = os.path.join('summaries', summary_descriptor)
            if not os.path.exists(path_to_summary):
                os.makedirs(path_to_summary, exist_ok=True)
