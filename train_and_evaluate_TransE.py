...
def train_TransE(dataset_path: str, entity_mapping, relation_mapping, experiments, output_directory):
    output_dir_path = Path(output_directory) / "TransE"
    ...
    for i, params in enumerate(experiments):
        ...
        try:
            log_file_path = Path(output_dir_path) / f"dim{params['embedding_dim']}_lr{params['lr']}_margin{params['margin']}_numNegs{params['num_negs']}.txt"
            result = pipeline(
                training=train_tf,
                validation=valid_tf,
                testing=test_tf,
                ...
                stopper='early',
                stopper_kwargs=dict(frequency=5, patience=3, relative_delta=0.002, metric='mean_reciprocal_rank'),
                result_tracker='csv',
                random_seed=42,
